...
public:
	Chessboard(std::unique_ptr<ChessInterface>&& chessEng) : chessEngine(std::move(chessEng))  {
...
public slots:
	void sendMove() {
		Move move;
		move.originPosition = SingletonMove::getSingleton()->originPosition;
		move.destinationPosition = SingletonMove::getSingleton()->destinationPosition;
